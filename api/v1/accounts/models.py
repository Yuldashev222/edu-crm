...
from api.v1.general import enums as general_enums, models as general_models
...
class User(AbstractBaseUser):
    ...
    role = models.CharField(max_length=20, choices=general_enums.ProfileRoles.choices())
    section = models.CharField(max_length=12, choices=general_enums.Sections.choices())
    ...
    days = MultiSelectField(choices=DAYS, max_length=20, blank=True, null=True)
    languages = models.ManyToManyField(general_models.Language, blank=True)
    status = models.CharField(max_length=20, choices=StudentStatus.choices(), blank=True, null=True)
    degree = models.CharField(max_length=10, choices=StudentDegrees.choices(), blank=True, null=True)
    ...
    date_end = models.DateField(help_text='date of completion of work or studies', blank=True, null=True)
    ...
    REQUIRED_FIELDS = ['company_id', 'phone_number', 'first_name', 'last_name']

    objects = MyAccountManager()

    gender = models.CharField(
        max_length=5,
        choices=general_enums.Gender.choices(),
        default=general_enums.Gender.man.value,
        null=True
    )
    birthday = models.DateField(validators=[validate_birthday], blank=True, null=True)
    ...
    # birth address
    region_1 = models.CharField(
        max_length=15,
        choices=general_enums.Regions.choices(),
        help_text='province of birth'
    )
