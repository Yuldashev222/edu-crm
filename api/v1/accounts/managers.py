...
from api.v1.general.enums import ProfileRoles, Sections
...
class MyAccountManager(BaseUserManager):
    ...
    def create_superuser(self, username, phone_number, company_id, first_name, last_name, password):
        ...
        user.section = Sections.managers.value
        user.save(using=self._db)
...
class GroupMemberManager(manager.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            role__in=[ProfileRoles.teacher.value, ProfileRoles.student.value, ProfileRoles.mentor.value])
